...
export const socketCtrl = {
	chatUpdate: async (req: CustomRequest, res: Response, _next: NextFunction) => {
		...
		try {
			if (!message.messages.some((el) => el.roomID === req.body.messageObj.roomID)) {
				...
				newMessage.save();
				let user = await UserModel.updateOne({ _id: req.body.userID }, { $push: { messages: newMessage._id } });
				let creator = await UserModel.updateOne({ _id: req.body.creatorID }, { $push: { messages: newMessage._id } });
		...
		try {
			...
			if (chat.messagesArr.length < 1) {
				try {
					...
					user.messages = await user.messages.filter((msg) => String(msg._id) !== String(chat._id));
					await user.save();
	...
	deleteChat: async (req: CustomRequest, res: Response, _next: NextFunction) => {
